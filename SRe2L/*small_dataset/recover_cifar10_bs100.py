...
def get_images(args, model_teacher, hook_for_display, ipc_id):
    ...
    targets_all = torch.LongTensor(list(range(10)) * 10)
...
def save_images(args, images, targets, ipc_id):
    targets = torch.LongTensor(np.arange(10))

    all_images = images # bs = 100

    for kk in range(10):
        images = all_images[kk * 10 : (kk + 1) * 10]
        ...
        for id in range(images.shape[0]):
            ...
            place_to_store = dir_path + "/class{:03d}_id{:03d}_kk{:03d}.jpg".format(class_id, ipc_id, kk)
